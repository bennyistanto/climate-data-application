...
"""
NAME
    STP_extract_spei_dry_month.py

    Sao Tome and Principe's SPEI-12 data extraction
...
EXAMPLES
    C:\\Program Files\\ArcGIS\\Pro\\bin\\Python\\envs\\arcgispro-py3\\python STP_extract_spei_dry_month.py
...
def create_DM_List(_DM_folder):
    ...
    print("looking for file with naming stp_cli_SPEI_dry_YYYYMMDD")
...
def create_monthly_List(_tif_folder):
    ...
    print("looking for file with naming stp_cli_SPEI_gamma_12_terraclimate_YYYY-MM-DD")
...
def execute_first_DM(_tiffolder, _DMFolder, threshold):
    ...
    first_data_name = 'stp_cli_SPEI_gamma_12_terraclimate_{0}-{1}-{2}.tif'.format(first_date[0:4], first_date[4:6], first_date[6:8])
    ...
    DMFilename = 'stp_cli_SPEI_dry_{0}{1}{2}.tif'.format(DMyear.zfill(4), DMmonth.zfill(2), DMday.zfill(2))
...
def execute_first_CDM(_tiffolder, _CDMFolder, threshold):
    ...
    CDMFilename = 'stp_cli_SPEI_dry_{0}{1}{2}.tif'.format(CDMyear.zfill(4), CDMmonth.zfill(2), CDMday.zfill(2))
...
def execute_DM(_lastdate, _tiffolder, _DM_folder, threshold):
    ...
    last_dryname = 'stp_cli_SPEI_dry_{0}'.format(_lastdate)
    last_dryfile = os.path.join(_DM_folder, last_dryname)
    next_monthlyname = 'stp_cli_SPEI_gamma_12_terraclimate_{0}-{1}-{2}.tif'.format(_lastdate[0:4], _lastdate[4:6], _lastdate[6:8])
    ...
    if arcpy.Exists(next_monthlydata):
        ...
        new_dry_name = 'stp_cli_SPEI_dry_{0}{1}{2}.tif'.format(DMyear1.zfill(4), DMmonth1.zfill(2), DMday1.zfill(2))
...
def execute_CDM(_lastdate, _tiffolder, _CDM_folder, threshold):
    ...
    if arcpy.Exists(next_monthlydata):
        ...
        new_dry_name = 'stp_cli_SPEI_dry_{0}{1}{2}.tif'.format(CDMyear1.zfill(4), CDMmonth1.zfill(2), CDMday1.zfill(2))
...
def execute_SUM(output_folder, folder_to_extract):
    ...
    sum_data_filename = 'stp_cli_SPEI_dry_duration_total_1958_2020.tif'
...
def execute_MAX(output_folder, folder_to_extract):
    ...
    max_data_filename = 'stp_cli_SPEI_dry_duration_max_1958_2020.tif'
...
if __name__ == '__main__':
    ...
    with arcpy.EnvManager(scratchWorkspace=r"X:\ArcGIS_TEMP\Scratch.gdb", \
        ...
        create_CDM('Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\consecutive_dry_month',\
            'Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\gamma',-1.2)
        # Run the function (output folder, input folder, threshold)
        create_DM('Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\dry_month',\
            'Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\gamma',-1.2)
	    # Run the function (output folder, input folder)
        execute_MAX('Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\consecutive_dry_month_max',\
            'Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\consecutive_dry_month')
        # Run the function (output folder, input folder)
        execute_SUM('Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\dry_month_total',\
            'Z:\\Temp\\TERRACLIMATE\\SPEI\\STP\\outputs\\geotiff\\dry_month')